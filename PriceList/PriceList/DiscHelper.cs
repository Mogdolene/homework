...
namespace PriceList
{
    public class DiscHelper
    {
            ...
            switch (Console.ReadLine())
            {
                case "1":
                {
                    ...
                    for (int i=0; i<_carriers.Count; i++)
                    {
                        if (_carriers[i].Name == criterion)
                        {
                            ...
                            PriceList.Program.xml.Delete(criterion, "name");
                        }
                    }
                    break;
                }
                case "2":
                {
                    ...
                    for (int i = 0; i < _carriers.Count; i++)
                    {
                        ...
                        {
                            _carriers.Remove(_carriers[i]);
                            PriceList.Program.xml.Delete(criterion, "manufacturer");
                        }
                    }
                    break;
                }
                case "3":
                {
                    ...
                    for (int i = 0; i < _carriers.Count; i++)
                    {
                        ...
                        {
                            _carriers.Remove(_carriers[i]);
                            PriceList.Program.xml.Delete(criterion, "model");
                        }
                    }
                    break;
                }
                case "4":
                {
                    ...
                    if (int.TryParse(criterion, out int result))
                    {
                        for (int i = 0; i < _carriers.Count; i++)
                        {
                            ...
                            {
                                _carriers.Remove(_carriers[i]);
                                PriceList.Program.xml.Delete(criterion, "quantity");
                            }
                        }
                    }
                    else
                ...
                case "5":
                {
                    ...
                    if (double.TryParse(criterion, out double result))
                    {
                        for (int i = 0; i < _carriers.Count; i++)
                        {
                            ...
                            {
                                _carriers.Remove(_carriers[i]);
                                PriceList.Program.xml.Delete(criterion, "price");
                            }
                        }
                    }
                    else
        ...
        public void Change()
        {
            ...
            if (j == "1")
            {
                _carriers[i].Quantity = int.Parse(val);
            }
            else
            {
                _carriers[i].Price = double.Parse(val);
            }
        ...
        {
            foreach (var informationCarrier in _carriers)
            {
                if (informationCarrier is FlashMemory)
                    PriceList.Program.xml.SaveFlash(informationCarrier as FlashMemory);
                if (informationCarrier is RemovableHDD)
                    PriceList.Program.xml.SaveHDD(informationCarrier as RemovableHDD);
                if (informationCarrier is DVDdisc)
                    PriceList.Program.xml.SaveDVD(informationCarrier as DVDdisc);
            }
        }
    }
}
